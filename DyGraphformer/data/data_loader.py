...
class Dataset_MTS(Dataset):
    def __init__(self, root_path, data_path='traffic.csv', flag='train', size=None,
                  data_split = [0.7, 0.1, 0.2], scale=True, scale_statistic=None):
        # size [seq_len, label_len, pred_len]，每一个batch取的输入长度和输出长度
        # info
        self.in_len = size[0]
        self.out_len = size[1]
        # init
        ...
        self.root_path = root_path   #数据集上级名称
        self.data_path = data_path   #数据集名称
    ...
    def __read_data__(self):
        ...
        if self.scale:
            if self.scale_statistic is None:
                self.scaler = StandardScaler()  #标准化处理

                train_data = df_data[border1s[0]:border2s[0]]
                self.scaler.fit(train_data.values)   #计算均值和方差
